...
import {RegisterFatmor, Assign, unAssign} from '../constants';

type useRegister = {
    register: (payload: RegisterFatmor) => Promise<void>;
    status: string;
    value: any;
    errorMessage: string;
    fieldErrors: any;
  ...
  export function useRegister(): useRegister {
    const {execute, status, value, errorMessage, fieldErrors} = useAsync<
      any,
      TYPES.RegisterFatmor
    >(services.register);
  
    async function register(payload: RegisterFatmor): Promise<void> {
      try {
